...
Function OpenInputStream:Stream(path:String)
	Local rawData:= __OS_Unsafe__LoadArray(RealPath(path))
	Local buffer:= New DataBuffer(rawData.Length)
	
	buffer.PokeBytes(0, rawData, 0, rawData.Length)
	
	Local S:= New DataStream(buffer)
	
	Return S
End

Function GetInfo_PNG:Int( path:String )
	Local f:=OpenInputStream(path)
